...
async def generate_tests(pr_metadata: dict, jira_ticket: dict) -> str:
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            logger.warning("Syntax error in generated test code", error=str(e), line=e.lineno)
            
            # Append history and ask for fix
            messages.append(AIMessage(content=response.content))
            messages.append(HumanMessage(content=f"The code you generated has a SyntaxError: {e} at line {e.lineno}. Please fix it and return ONLY the corrected python code."))
