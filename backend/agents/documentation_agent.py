...
async def generate_documentation(pr_metadata: dict, jira_ticket: dict, findings: list) -> dict:
    """
    ...
    """
    logger.info("Generating inline documentation for PR", pr_title=pr_metadata.get('title'))
    ...
    messages = [
        ...
        HumanMessage(content=build_human_message(pr_metadata, jira_ticket, findings, raw_files))
    ]
    ...
    try:
        ...
        logger.info("Successfully parsed inline documentation", num_files=len(updated_files))
    except json.JSONDecodeError as e:
        logger.error("Failed to parse JSON from documentation agent", error=str(e), response=content)
